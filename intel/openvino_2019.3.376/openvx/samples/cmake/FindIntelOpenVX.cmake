...
if(NOT IntelOpenVX_ROOT_DIR)
    ...
    elseif(DEFINED INTEL_OPENVINO_DIR)
        set(IntelOpenVX_ROOT_DIR "${INTEL_OPENVINO_DIR}" CACHE PATH "${IntelOpenVX_ROOT_DIR_DESCRIPTION}" FORCE)
...
#get INTEL_OPENVINO_DIR from enviroment if it is not defined yet
if(NOT DEFINED INTEL_OPENVINO_DIR AND DEFINED ENV{INTEL_OPENVINO_DIR})
    set(INTEL_OPENVINO_DIR "$ENV{INTEL_OPENVINO_DIR}")
endif()
if(NOT DEFINED INTEL_OPENVINO_DIR AND DEFINED ENV{IAP_SDK_DIR})
    set(INTEL_OPENVINO_DIR "$ENV{IAP_SDK_DIR}")
endif()
...
util_find_dir(IntelOpenVX_INCLUDE_DIR
    ...
    if (WIN32)
        ...
        find_library(IntelOpenVX_IAP_OPENVX_LIB     iap_openvx     libiap_openvx.so.1.1.0     HINTS ${ovx_lib_dirs} NO_DEFAULT_PATH)
    endif()

    if(USE_INTEL_OLDER_EXT)
    find_library(IntelOpenVX_IAP_OPENVX_EXT_LIB iap_openvx_ext libiap_openvx_ext.so.1.1.0 HINTS ${ovx_lib_dirs} NO_DEFAULT_PATH)
    endif()
    ...
    if (WIN32)
        ...
        find_path(IntelOpenVX_IAP_OPENVX_RUNTIME_DIR
            "${CMAKE_SHARED_LIBRARY_PREFIX}iap_openvx${CMAKE_STATIC_LIBRARY_SUFFIX}"
            "${CMAKE_SHARED_LIBRARY_PREFIX}iap_openvx${CMAKE_STATIC_LIBRARY_SUFFIX}.1.1.0"
            PATHS ${ovx_lib_dirs}
            NO_DEFAULT_PATH)
    else()
        find_path(IntelOpenVX_IAP_RUNTIME_DIR
        ...
        find_path(IntelOpenVX_IAP_OPENVX_RUNTIME_DIR
            ...
            "${CMAKE_SHARED_LIBRARY_PREFIX}iap_openvx${CMAKE_SHARED_LIBRARY_SUFFIX}.1.1.0"
            PATHS ${ovx_lib_dirs}
            NO_DEFAULT_PATH)
    endif()
    find_path(IntelOpenVX_IAP_OPENVX_EXT_RUNTIME_DIR
        "${CMAKE_SHARED_LIBRARY_PREFIX}iap_openvx_ext${CMAKE_SHARED_LIBRARY_SUFFIX}"
        "${CMAKE_SHARED_LIBRARY_PREFIX}iap_openvx_ext${CMAKE_SHARED_LIBRARY_SUFFIX}.1.1.0"
        PATHS ${ovx_lib_dirs}
        NO_DEFAULT_PATH)
