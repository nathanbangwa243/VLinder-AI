...
typedef struct _VX_MD_Session
{
    ...
    vx_enum       heterogeneity;// A flag to set OpenVX Nodes to run on heterogenous hardware platforms: CPU, GPU, IPU, etc.
    vx_uint32     sceneAdaption = 150; // The scene apaption vaule
...
vx_status assignNodeTarget(vx_context context, vx_node node, const char* nodename, vx_enum heterogeneity)
{
    ...
    vx_enum target_cpu    = VX_TARGET_CPU_INTEL;
    vx_enum target_gpu    = VX_TARGET_GPU_INTEL;
    vx_enum target_ipu    = VX_TARGET_IPU_INTEL;

    switch(heterogeneity)
    {
        case WL_HETER_CPU_ONLY:
            status = vxSetNodeTarget(node, target_cpu, 0);
            if (status == VX_SUCCESS)
                IntelVXSample::logger(0) << "[ INFO ] " << nodename << " is assigned to CPU" << '\n';
            else
                IntelVXSample::logger(0) << "[ INFO ] " << nodename << " failed to assign to CPU" << '\n';
            CHECK_VX_STATUS(status);
            break;

        case WL_HETER_GPU_ONLY:
            status = vxSetNodeTarget(node, target_gpu, 0);
            if (status == VX_SUCCESS)
                IntelVXSample::logger(0) << "[ INFO ] " << nodename << " is assigned to GPU" << '\n';
            else
                IntelVXSample::logger(0) << "[ INFO ] " << nodename << " failed to assign to GPU" << '\n';
            CHECK_VX_STATUS(status);
            break;

        case WL_HETER_IPU_ONLY:
            status = vxSetNodeTarget(node, target_ipu, 0);
            if (status == VX_SUCCESS)
                IntelVXSample::logger(0) << "[ INFO ] " << nodename << " is assigned to IPU" << '\n';
            else
                IntelVXSample::logger(0) << "[ INFO ] " << nodename << " failed to assign to IPU" << '\n';
            CHECK_VX_STATUS(status);
            break;
    ...
    std::vector<vx_node>    &vxNodes)
{
    ...
    // Step 2: scale image
    scaleImageNodes[0] = vxScaleImageNode(graph, scaleImages[0], scaleImages[3], VX_INTERPOLATION_BILINEAR);
    ...
    scaleImageNodes[1] = vxScaleImageNode(graph, scaleImages[1], scaleImages[4], VX_INTERPOLATION_BILINEAR);
    ...
    scaleImageNodes[2] = vxScaleImageNode(graph, scaleImages[2], scaleImages[5], VX_INTERPOLATION_BILINEAR);
...
vx_graph CreateMotionDetectionGraph(
    vx_context              context,
    ...
    vx_uint32               sceneAdaption,  // scene adaption value
    ...
    std::vector<vx_array>   &vxArrays)      // vector of arrays
{
     ...
     */
    vx_bg_state_intel bgState = vxCreateBGStateIntel(context, internalWidth, internalHeight, sceneAdaption, 5, 17.0*17.0f,
                                         4.0*4.0f, 25.0*25.0f,  25.0f, 0.9f, 12.0f, 0.95f, 1, 0, 0.5);
...
static WL_Status vx_md_init (void *workload)
{
    ...
    CHECK_VX_STATUS(vxLoadKernels(md->context, "motion_detection_user_nodes_module"));

    md->graph = CreateMotionDetectionGraph(md->context, md->width, md->height, color, md->threshold, md->scaleImage, md->scaleFlag, md->heterogeneity, md->heterogeneity_config_file, md->scaleFactor, md->sceneAdaption, md->vxNodes, md->vxImages, md->vxArrays);
    if( md->graph == NULL)
    {
        std::cerr << "[ ERROR ] Failed to CreateMotionDetectionGraph: " << "\n";
...
static WL_Status vx_md_config (void *workload, WL_Cfg_Index index, void* config)
{
    ...
    switch (index) {
      ...
      case WL_CFG_MOTIONDETECTION:
      {
          ...
          if ((mdConfig->sceneAdaption < 50) || (mdConfig->sceneAdaption > 200))
          {
            std::cout << "*** Warning: detect unreasonable scene adaption value " << mdConfig->sceneAdaption << ". Set it carefully unless you know clearly what you did! ***\n";
          }

          md->threshold     = mdConfig->threshold;
          md->mergeBoxes    = mdConfig->mergeBoxes;
          md->scaleImage    = mdConfig->scaleImage;
          md->sceneAdaption = mdConfig->sceneAdaption;
          break;
      }
      ...
      case WL_CFG_HETEROGENEITY:
      {
          ...
          if((hetero_info->heterogeneity_pref <= WL_HETER_UNKNOWN) || (hetero_info->heterogeneity_pref >= WL_HETER_MAX_NUM))
          {
...
static WL_Status vx_md_process (void *workload, WL_Images *inbufs, WL_Images *outbufs, void* outmetadata)
{
    ...
    CHECK_VX_STATUS(vxCopyImagePatch(md->vxImages.front(), &rect, plane, &addr, base_ptr, VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST));
    ...
    CHECK_VX_STATUS(vxQueryArray(md->vxArrays.back(), VX_ARRAY_NUMITEMS, &numItems, sizeof(numItems)));

    if (numItems != 0)
    {
        vx_map_id map_id;
        CHECK_VX_STATUS(vxMapArrayRange(md->vxArrays.back(), 0, numItems, &map_id, &stride, reinterpret_cast<void**>(&pObjectListPtr), VX_READ_ONLY, VX_MEMORY_TYPE_HOST, 0));

        objectList.reserve(numItems);
        std::copy(pObjectListPtr, pObjectListPtr + numItems, std::back_inserter(objectList));

        CHECK_VX_STATUS(vxUnmapArrayRange(md->vxArrays.back(), map_id));
