...
class Component:
    def __init__(self, name, properties, logger):
        ...
        for k, v in properties.items():
            setattr(self, k, str(v, 'utf-8') if type(v) == bytes else v)
        self.available = True
        self.invisible = True if 'ui_name' not in properties else False
        self.selected = True if not self.invisible else False
        self.logger = logger
    ...
    def copy_files(self, destination):
        if not self.is_exist():
            raise FileNotFoundError("Files for component {} not found. "
                                    "Please check your OpenVINO installation".format(self.ui_name))
        else:
...
class ComponentFactory:
    ...
    def create_component(name, properties, logger):
        return globals()["Component"](name, properties, logger)


# class that operating with JSON configs
class ConfigReader:
    def __init__(self, logger):
        cfg_path = os.path.join(os.path.dirname(__file__), os.pardir, "configs/main.json")
        if os.path.isfile(cfg_path):
            logger.info("Loading main.cfg...")
            ...
            raise RuntimeError("Main config can't be found at {}".format(os.path.abspath(cfg_path)))
