...
class UserInterface:
    ...
    def run(self):
        ...
        finalization_map = [
            ...
            {
                "shortcut": "u",
                "message": "Provide(or change) path to folder with user data(IRs, models, etc) "
                           "[ {} ]".format(self.args.user_data),
        ...
        us_hdrs = ["Deployment Manager\nVersion " + self.version,
                   self.separator, "Review the configuration settings below.\n"
                                   "Remove or add items by typing the number and "
                                   "hitting \"Enter\".",
                   self.separator, "You may select/deselect components\n"]
        self.us_buttons = Controls(user_selection_map)
        us_imsg = self.separator + "\nPlease type a selection or press \"Enter\" " \
                                   "to accept default choice [ select all ]"
