...
int _send_buffer_to_dev(hidapi_device_s* handle, unsigned char* read_write_buff) {
  int ret = hidapi_write(handle, read_write_buff, HID_PACKET_SIZE);
  if (ret <= 0) {
    BslLog(BSL_LOG_ERROR, "hidapi_write failed");
    return -1;
  }
  BslLog(BSL_LOG_DEBUG, "write ret=%d", ret);
  ...
  {
    BslLog(BSL_LOG_DEBUG, "failed to poll data from f75114");
    return -1;
  } else {
    /* Check for errors on the file descriptor. This will
     indicate a device disconnection. */
    if (fds.revents & (POLLERR | POLLHUP | POLLNVAL)) {
      BslLog(BSL_LOG_ERROR, "device maybe disconnect");
      return -1;
    }
  }
  ...
  int length = hidapi_read(handle, read_write_buff, HID_PACKET_SIZE);

  BslLog(BSL_LOG_DEBUG, "read length: %d", length);
...
int _send_init_cmd(hidapi_device_s* handle) {
  ...
  if (ret != 0 || buf[0] != 0xFF) {
    BslLog(BSL_LOG_DEBUG, "read result failed;ret=%d;buf[0]=%x", ret, buf[0]);
    return -1;
  }
  return 0;
}
...
int _send_output_cmd_with_buf(hidapi_device_s* handle, int pin_id, int func_id, unsigned char* buf) {
  ...
  if (ret != 0 || buf[0] != 0x80) {
    BslLog(BSL_LOG_ERROR, "read result failed;ret=%d;buf[0]=%x", ret, buf[0]);
    return -1;
  }
...
int _send_pull_mode_cmd(hidapi_device_s* handle, int pin_id, eGPIO_Pull_Mode eMode) {
  if (eMode < eGPIO_Pull_Disable) {
    ...
    if (ret) {
      BslLog(BSL_LOG_ERROR, "send input cmd failed");
      return ret;
    }
    buff[3] = buff[0];
    BslLog(BSL_LOG_DEBUG, "pre-process buff[3]: %x", buff[0]);
    if ((pin_id & 0x0f) == 0 || (pin_id & 0x0f) == 4)
      ...
      buff[3] &= 0x3f;
    BslLog(BSL_LOG_DEBUG, "%s: post-process buff[3]: %x", buff[0]);
    buff[0] = 0xF0;  // For all registry
    buff[1] = 0x01;  // write
    buff[2] = addr;
    ...
    if (ret) {
      BslLog(BSL_LOG_ERROR, "send output cmd failed");
      return ret;
    }
    return 0;
  }
...
BSL_STATUS _f75114_init() {
  for (int i = 0; i < m_hid_count; i++) {
    ...
    for (j = 0; j < F75114_MAX_BOARDID_PIN_NUM; j++) {
      ...
      if (ret) {
        BslLog(BSL_LOG_ERROR, "read board id failed:%d", ret);
      }
      BslLog(BSL_LOG_DEBUG, "board id bit %d:%d", j, value);
      boardid |= ((int)value & 0x1) << j;
    }

    BslLog(BSL_LOG_DEBUG, "board id is:%d", boardid);
...
static BSL_STATUS m_hid_find_resetpin_by_deviceid(int real_device_id, int* pin_id) {
  int i = 0;
  BslLog(BSL_LOG_DEBUG, "m_hid_find_resetpin_by_deviceid real_device_id=%d", real_device_id);
...
static BSL_STATUS m_hid_discard(int device_id) {
  UNUSED(device_id);
  BslLog(BSL_LOG_WARNING, "Device Discard for HID F75114 is not supported");
  return BSL_ERROR_UNSUPPORTED_FUNCTION;
}

static BSL_STATUS m_hid_reset_all() {
  if (m_hid_count == 0) {
    BslLog(BSL_LOG_ERROR, "no F75114 register, need a scan?");
...
void log_hidapi_device_info(const struct hidapi_device_info_s* cur_dev) {
  BslLog(BSL_LOG_DEBUG, "Device Found:");
  BslLog(BSL_LOG_DEBUG, "  VID:PID:        %04hx:%04hx", cur_dev->vendor_id, cur_dev->product_id);
  BslLog(BSL_LOG_DEBUG, "  Path:           %s", cur_dev->path);
  BslLog(BSL_LOG_DEBUG, "  Serial Number:  %ls", cur_dev->serial_number);
  BslLog(BSL_LOG_DEBUG, "");
  BslLog(BSL_LOG_DEBUG, "  Path:           %s", cur_dev->path);
  BslLog(BSL_LOG_DEBUG, "  Manufacturer:   %ls", cur_dev->manufacturer_string);
  BslLog(BSL_LOG_DEBUG, "  Product:        %ls", cur_dev->product_string);
  BslLog(BSL_LOG_DEBUG, "  Release:        %hx", cur_dev->release_number);
  BslLog(BSL_LOG_DEBUG, "  Interface:      %d", cur_dev->interface_number);
  BslLog(BSL_LOG_DEBUG, "");
}

static BSL_STATUS m_hid_scan_impl(int* p_device_count, const char* parent_path_list[], size_t path_num) {
  m_hid_count = 0;
  BslLog(BSL_LOG_INFO, "scan F75114 device...");
  ...
  *p_device_count = m_hid_count;
  BslLog(BSL_LOG_INFO, "found %d F75114 device", m_hid_count);
